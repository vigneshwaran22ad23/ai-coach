...
import sqlite3, hashlib, json, os, requests, re, traceback
...
# Catch-all: ensures every error (even ones outside our own try/except blocks)
# comes back as JSON. Without this, a crash on a serverless host (e.g. Vercel)
# returns a plain-text "Internal Server Error" page, which breaks the
# frontend's JSON.parse() with "Unexpected token 'I', 'Internal S'...".
...
# except for /tmp. SQLite needs a writable file, so we redirect the DB there
# when running on Vercel. NOTE: /tmp is ephemeral — it can be wiped between
# cold starts, so quiz history/progress will NOT reliably persist in
# production on Vercel. Fine for a demo; use Railway/Render (persistent
# disk) if you need real data persistence.
DB = "/tmp/coach.db" if os.environ.get("VERCEL") else "coach.db"
